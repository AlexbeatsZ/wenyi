...
- 阅读页打开时读取一次；之后仅在用户手动查询时读取全书进度，且仅当当前章节的文件修订时间发生变化时重载正文。
- 手机端浏览器 QA 发现窄屏下手动查询按钮会被挤压，已为按钮设置固定宽度；390×844 视口下章节切换、查询和字号切换均正常。
- 贴吧逐章发布使用 `tieba-publish`：默认仅预览，正文从首个 heading 起跳过 EPUB 日期/字数元数据；真实页面上限为 2000 字，默认按 1950 字在段落边界拆层并平衡过短尾层。实际发送必须显式传 `--publish`，使用项目 `state/publish/tieba-chrome-profile` 内持久登录态、60–75 秒限速、单实例文件锁和 `state/<书名>/publish/` 原子断点；发布后刷新并切倒序，只以服务器持久楼层为成功信号，接受贴吧等长 `*` 过滤，自动对账 `submitting`，不读取日常 Chrome 配置，也不绕过验证码。
- Windows 后台启动 reader 并重定向日志时，控制台默认 GBK 会因书名中的 `〜` 在启动提示处抛出 `UnicodeEncodeError`；只需为 reader 子进程设置 `PYTHONIOENCODING=utf-8`，无需修改全局编码或译文状态。
...
- 与用户讨论顺读中的翻译问题时，用户指出的章节和具体段落是严格剧透边界。可以在后台查后文核验指代，但答复与译文说明只能使用截至该段已经出现的信息；后文人物身份、关系和情节一律不得透露，应使用“第一位朋友 / 第二位朋友”等当前位置可理解的匿名指代。
...
- 对真实失败批次的无落库复现中，Gemini Pro 与 Codex Sol 均能返回严格等长结果，说明历史缺口不是稳定的敏感内容拒绝，更符合暂态坏 JSON、漏项或 CLI 异常。恢复策略应让 Gemini 保持主精修模型：批级失败先递归二分重试，只把仍失败的叶子交给独立 `polish_fallback_llm`，并持久化失败类型、递归范围与备用索引。
- 恢复代码合入后的正式续跑从 176 个 pending 精修段开始：ch62/ch79/ch131 直接由 Gemini 补完，ch109 递归拆分 3 个范围、ch111 拆分 1 个范围后也由 Gemini 补完；Codex 精修备用实际接管 0 段。由此验证“先缩小失败批次、保留 Gemini 主模型”的策略有效，且没有不必要地换模型。
...
- [x] 新增 `reader` 局域网移动阅读器：暗色排版、手动查询、章节导航、字号调节和原文对照开关。
...
- [x] 已清理首次翻译状态，仅保留第二次干净重译于 `state/<书名>`；默认 `config.yaml` 读取该正式状态，普通 `trans-novel status/translate` 不会误显示旧成品状态。
...
- [x] 2026-07-30 修正第 118 话小雪与奈央的年龄对话：依据后续“同性恋、刚失恋”及第 125 话揭晓，将小雪所指明确为“那个像家人一样的朋友”（尾关君香），并将奈央的确认改为“原来是位比你年长的朋友啊？”。报告与 EPUB 已重组，旧句归零、新句各命中 1 次，141 文件结构测试通过；当前 EPUB SHA-256 为 `59287B9555553B63E31C1112C533DDBCBE073787DBEFF8D4DE24D8FDF3902667`。修正前备份位于 `%LOCALAPPDATA%\Temp\.agents\wenyi-ch118-dialogue-20260730`。
- [x] 2026-07-30 将同一家 `焼き鳥屋` 的“烧鸟店 / 烤串店 / 烤鸡肉串店”统一为“烤鸡串店”，覆盖 ch32/61/72/122/134/135 共 15 处，避免读者误认成不同店铺；并将 ch135 的 `閉店時間を気にしている` 明确为奈央以为前辈在盘算能否赶在店铺打烊前过去，而非操心店铺营业。EPUB 中旧称与旧句均归零，141 文件结构测试通过。当前 EPUB SHA-256 为 `7D23022B4AF2DC346977A2E733F225E975629B6C981F0E02E7C598924760A038`，修正前备份位于 `%LOCALAPPDATA%\Temp\.agents\wenyi-yakitori-term-20260730`。
- [x] 2026-07-30 新增 `tieba-publish` 逐章发布：已按真实主题 `10905826072` 的当前回复框结构验证选择器；真实提交确认网页端上限为 2000 字，当前成品按 1950 字安全上限重排为 136 话、359 层、约 57 万字符。新增格式化/拆层/断点测试，完整测试 310 项与 15 个子测试通过，仅余 2 项既有 Windows `/tmp/output` 路径断言失败。
- [ ] 贴吧主题 `10905826072` 正由新版后台继续发布：`ch4-part2` 因旧版正序 DOM 误判而重复；保留原楼层 `153776746371`，较新重复楼层 `153776769584` 按用户决定暂不由脚本删除。后台持久核验至 `ch49-part1` 后，`ch49-part2` 提交后的单次刷新页未渲染倒序控件而停止；2026-07-31 00:05 加入验证页最多 5 次重新导航后重启，自动确认该层已经持久保存并恢复为 `posted`，随后 `ch50-part1` 已发布并核验成功。新版使用倒序刷新、星号过滤兼容、`submitting` 自动对账、缺失冷却后单次重试、回复框刷新重试、验证页恢复重试和单实例锁。当前日志为 `%LOCALAPPDATA%\Temp\.agents\wenyi-tieba-publisher-run\publish-20260731-000550.*.log`，断点位于本书状态目录的 `publish/tieba-10905826072.json`。
