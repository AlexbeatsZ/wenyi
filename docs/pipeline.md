...
- **Final review:** starts only after every chapter has been translated. When configured, the main model first arbitrates unresolved glossary candidates from local source/translation context. Each chapter then derives its relevant term snapshot from that completed glossary. Chapters are divided into contiguous chunks and checked in parallel; when `autofix_severe` is enabled, severe fixes use the same `review_llm` that found the issues (or the main `llm` when no separate reviewer is configured).
