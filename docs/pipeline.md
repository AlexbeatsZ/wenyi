...
- **Refinement recovery:** malformed, empty, misaligned, and provider-failed refinement batches are recursively split and retried with the main model. Only a failed single-segment leaf uses `polish_fallback_llm`; failures and fallback indexes are persisted for diagnosis.
...
- **Final review:** starts only after every chapter has been translated. When configured, the main model first arbitrates unresolved glossary candidates from local source/translation context. Each chapter then derives its relevant term snapshot from that completed glossary. Chapters are divided into contiguous chunks and checked in parallel. Terminology candidates must cite an exact source mapping; aliases only retrieve context and never inherit a whole-string target. The reviewer also reports objectively broken Chinese as `fluency`, without automatically rewriting it.
- **Shadow fix validation:** when `autofix_severe` is enabled, severe fixes use the same `review_llm` that found the issues. The proposal remains shadow text until a fresh review, without the old feedback, finds no supported issue; failure preserves the formal translation and records the rejected proposal.
- **Stage archive:** `artifacts/translations/chN.jsonl` append-only records first drafts, refinements, punctuation normalization, review proposals, and adopted fixes. `artifacts/inputs/<sha256>.json` deduplicates the exact structured context for a batch. `events.jsonl` links these artifacts with model, fallback, failure, and validation metadata.
