...
本机已安装并登录 Antigravity CLI 时，可把每次请求作为非交互普通提示词发送：
...
`gemini-3.6-flash[-low|-medium|-high]` 这组短 ID 会优先传给 agy 1.1；只有 CLI
明确报告模型名不识别时，先重试一次短 ID，再回退到 agy 1.0 接受的显示名，
并缓存成功选择。这可容忍 agy 1.1 启动时模型注册表尚未就绪的短暂竞态。
...
`--resolve-conflicts` 或 `--no-resolve-conflicts` 会覆盖术语冲突自动裁定开关。
