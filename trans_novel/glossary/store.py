...
def _term_occurs(text: str, key: str) -> bool:
    ...
    Japanese has no universal whitespace boundary, but short kana names and
    one-kanji names are especially prone to substring false positives:
    ``あんな`` in ``あんなに``, ``メイ`` in ``メイン``, ``明`` in
    ``明るい`` and ``母`` in ``お母さん``.  Longer/mixed keys keep the old
    substring behaviour because compounds and attached particles are normal.
    """
    if not key:
        return False
    start = 0
