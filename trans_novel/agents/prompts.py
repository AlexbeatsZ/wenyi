...
- pronoun：人称/性别代词错误
性别/人称只有在当前原文明示，或对照表提供已经确认的性别事实时才能判错；姓名印象、外貌、服装、语气、
...
人物性别未由当前原文明示或对照表确认时，不得根据姓名印象、外貌、服装、语气或第一人称猜测；
优先重复姓名/称谓、自然省略主语或使用不暴露性别的表达。全书概览中的后文真相不得提前泄露，
...
仅输出 JSON：
{
  ...
  "characters": [{"source":"原文名","reading":"读音(可空)","target":"建议中文译名","gender":"男/女/未知","gender_confidence":"confirmed/suspected/unknown","gender_evidence":"原文中的直接证据；没有则留空","note":"性格/语气特征，须包含说话方式：自称、口癖、敬语习惯"}],
...
2. 同一实体的稳定称呼变体：昵称、外号、缩写、大小名等优先放入该实体的 aliases；只有译文确实采用了与本名不同、需要全书统一的独立译法时才输出「称谓」条目。
