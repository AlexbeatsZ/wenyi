...
- AGY/Gemini 的长 JSON 偶发返回无效结构；`parse_json_loose` 的错误文本只展示回复前 200 字符，不能据此断言原回复固定被截到 200 字符。真实失败章节与剩余术语批次原样重放均能成功，合成 60 项/6362 字节 JSON 也正常，根因是暂态坏回复，而当前 Reviewer/GlossaryArbiter 缺少 JSON 解析失败后的重试或递归拆批。
...
- [ ] 《屈曲ラヴァー》最终审校因两次暂态无效 JSON 中止：当前 26 章 done、1 章 failed、110 章 pending；术语冲突剩 11 条记录/9 个独立术语，前 45 个模型裁定已安全落库。可重跑相同模式续审，但仍待实现 JSON 解析失败后的自动重试/拆批。
