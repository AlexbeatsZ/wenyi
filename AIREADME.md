...
- 贴吧逐章发布使用 `tieba-publish`：默认仅预览，正文从首个 heading 起跳过 EPUB 日期/字数元数据；真实页面上限为 2000 字，默认按 1950 字在段落边界拆层并平衡过短尾层。实际发送必须显式传 `--publish`，使用项目 `state/publish/tieba-chrome-profile` 内持久登录态、60–75 秒限速和 `state/<书名>/publish/` 原子断点；每层提交后会核对章节标记唯一且正文完整，不读取日常 Chrome 配置，也不绕过验证码。
...
- [ ] 贴吧主题 `10905826072` 发布中：第 1 话两层已人工试发并核对；第 2–136 话由独立 Chrome 后台自动发布。截至 2026-07-30 14:45，第 4 话第 2 层首次提交后未出现，脚本安全停止；只读复核确认主题中不存在该标记后已允许重试，重试楼层唯一、正文完整并记为 `posted`，脚本正从断点继续剩余层。当前运行日志位于 `%LOCALAPPDATA%\Temp\.agents\wenyi-tieba-publisher-run\publish-20260730-144449.*.log`，断点位于本书状态目录的 `publish/tieba-10905826072.json`。
