...
- [ ] 贴吧主题 `10905826072` 正由新版后台继续发布：`ch4-part2` 因旧版正序 DOM 误判而重复；保留原楼层 `153776746371`，较新重复楼层 `153776769584` 按用户决定暂不由脚本删除。首轮新版后台连续持久核验至 `ch43-part1`，随后因回复框偶发未渲染而停止；2026-07-30 23:12 重启后自动确认 `ch43-part2` 未落库，安全重试并核验成功，现继续剩余层。新版使用倒序刷新、星号过滤兼容、`submitting` 自动对账、缺失冷却后单次重试、回复框刷新重试和单实例锁。当前日志为 `%LOCALAPPDATA%\Temp\.agents\wenyi-tieba-publisher-run\publish-20260730-231233.*.log`，断点位于本书状态目录的 `publish/tieba-10905826072.json`。
