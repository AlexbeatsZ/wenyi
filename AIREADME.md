...
- 贴吧逐章发布使用 `tieba-publish`：默认仅预览，正文从首个 heading 起跳过 EPUB 日期/字数元数据，超长章按段落拆层；实际发送必须显式传 `--publish`，使用独立 Chrome 配置、60–75 秒限速和 `state/<书名>/publish/` 原子断点，不读取日常 Chrome 配置，也不绕过验证码。
...
- [x] 2026-07-30 新增 `tieba-publish` 逐章发布：已按真实主题 `10905826072` 的当前回复框结构验证选择器；当前成品预览为 136 话、143 层、567,402 字符（默认 8000 字符上限），未实际发帖。新增格式化/拆层/断点测试，完整测试 310 项与 15 个子测试通过，仅余 2 项既有 Windows `/tmp/output` 路径断言失败。
